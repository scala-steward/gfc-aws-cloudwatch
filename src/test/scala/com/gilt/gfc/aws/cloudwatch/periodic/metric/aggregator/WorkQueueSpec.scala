...
class WorkQueueSpec
  ...
  "WorkQueue" should {
    ...
    "drain non-empty queue into an iterator" in {
      ...
      it.toList.groupBy(_._1).mapValues(_.map(_._2)).toSeq.map(_.toString).sorted.toString shouldEqual(
        "ArrayBuffer((nsBar,List({Dimensions: [],Value: 1.0,Unit: bar1}, {Dimensions: [],Value: 1.0,Unit: bar2})), (nsFoo,List({Dimensions: [],Value: 1.0,Unit: foo1}, {Dimensions: [],Value: 1.0,Unit: foo2})))"
      )
    }
  }
}
